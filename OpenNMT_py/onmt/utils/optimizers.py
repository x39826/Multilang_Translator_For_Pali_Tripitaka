...
def f(model):
    def diff_dict(d1,d2):
        ...
        for k in df:
            diff[k] = a[k]
        return diff, un
    p1 , p2 = diff_dict(model.named_parameters(), model.generator.named_parameters())
    #optim.set_parameters(p2, p1)
    return p1, p2
...
def build_torch_optimizer(model, opt):
    ...
    if opt.optim == 'sgd':
        ...
        generator = []
        p1,p2 = f(model)
        for name, param in p1.items():#model.named_parameters():
        ...
        optimizer = MultipleOptimizer(
             ...
             optim.SparseAdam(
                 ...
                 betas=betas,
                 eps=1e-8),
             optim.SGD(generator, lr=opt.learning_rate)]
            )
