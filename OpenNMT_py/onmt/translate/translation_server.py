...
class ServerModel(object):
    ...
    def load(self):
        ...
        if self.tokenizer_opt is not None:
            ...
            if self.tokenizer_opt['type'] == 'sentencepiece':
                ...
                sp.Load(model_path)
                self.tokenizer = sp
            if self.tokenizer_opt['type'] == 'multi-sentencepiece':
                if "model" not in self.tokenizer_opt:
                    raise ValueError(
                        "Missing mandatory tokenizer option 'model'")
                import sentencepiece as spm
                ...
                model_path = os.path.join(self.model_root,
                                          self.tokenizer_opt['model'])
                for _, f in enumerate(glob.glob(r'%s/*.model'%model_path)):
                    lang_name  = f.split('/')[-1].split('.')[0]
                    sp = spm.SentencePieceProcessor()
                    sp.Load(f)
                    self.tokenizer[lang_name] = sp
    ...
    def tokenize(self, sequence):
        ...
        if self.tokenizer_opt["type"] == "multi-sentencepiece":
            #'''
            src2tgt = re.findall(r'^.*?\s', sequence)[0].strip()
            content = re.split(r'^.*?\s', sequence)[1]
            f_lang, t_lang = src2tgt.split('|')
            tok = self.tokenizer[f_lang].EncodeAsPieces(content)
            tok = " ".join(tok)
            tok = '<' + t_lang + '> ' + tok
            #'''
        elif self.tokenizer_opt["type"] == "pyonmttok":
    ...
    def detokenize(self, sequence):
        ...
        if self.tokenizer_opt["type"] == "multi-sentencepiece":
            #detok = re.split(r'^.*?\s', sequence)[1]
            detok = sequence.replace(' ','').replace('▁',' ')
        elif self.tokenizer_opt["type"] == "pyonmttok":
