...
class Translator(object):
    ...
    def translate(
        ...
        """
        if trans_to != "":
            trans_to = '<t_' + trans_to + '>'
            if trans_to in self._tgt_vocab.stoi:
                self.trans_to = self._tgt_vocab.stoi[trans_to]
            else:
                print("language name is %s invalid"%trans_to)
                exit(0)
        else:
            self.trans_to = None
    ...
    def translate_batch(self, batch, src_vocabs, attn_debug):
        ...
        with torch.no_grad():
            if self.trans_to is not None:
                lang_id = batch.src[0][0,:,:]
                batch.src[0].narrow(0,0,1).fill_(self.trans_to)
